...
def require_faculty(f):
    """Decorator to require faculty role (faculty, hod, admin)"""
    return require_role(['faculty', 'hod', 'admin'])(f)
...
def can_approve_request(request_obj):
    ...
    # Check workflow permissions: Guide → HOD → IT Services → Admin
    from backend.models.project_request import StatusEnum
    
    if current_user.role == RoleEnum.project_guide and request_obj.status == StatusEnum.pending:
        return True, "Project Guide can approve pending requests"

    if current_user.role == RoleEnum.hod and request_obj.status == StatusEnum.guide_approved:
        return True, "HoD can approve guide approved requests"

    if current_user.role == RoleEnum.it_services and request_obj.status == StatusEnum.hod_approved:
        return True, "IT Services can approve HOD approved requests"
...
def get_user_approval_queue():
    ...
    if current_user.is_admin:
        return ProjectRequest.query.filter_by(status=StatusEnum.it_services_approved)

    # IT Services sees HOD approved
    if current_user.role == RoleEnum.it_services:
        return ProjectRequest.query.filter_by(status=StatusEnum.hod_approved)

    # HoD sees Guide approved
    if current_user.role == RoleEnum.hod:
        return ProjectRequest.query.filter_by(status=StatusEnum.guide_approved)

    # Project Guide sees Pending
    if current_user.role == RoleEnum.project_guide:
