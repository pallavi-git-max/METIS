...
def dashboard():
    ...
    try:
        ...
        hod_submitted = ProjectRequest.query.filter_by(
            user_id=current_user.id, 
            status=StatusEnum.hod_submitted
        ).count()
...
def notifications():
    ...
    try:
        ...
        for request_obj in recent_requests:
            ...
            elif request_obj.status == StatusEnum.hod_submitted:
                notifications.append({
                    'id': f"req_{request_obj.id}_hod_submitted",
                    'type': 'info',
                    'title': 'Request Forwarded',
                    'message': f'Your request "{request_obj.project_title}" has been forwarded to faculty.',
